...
public class vp_Bullet : MonoBehaviour
{
	...
	public float Damage = 100.0f;					// the damage transmitted to target by the bullet
	...
	protected virtual void DoHit()
	{
        ...
        {
            Debug.Log("You fcking hit a zombie mofo");
        }
    ...
    private ZombieHandler GetZombieHandler(GameObject obj)
    {
        if (obj.GetComponent<ZombieHandler>() != null) return obj.GetComponent<ZombieHandler>();
        else if (obj.transform.parent != null) return GetZombieHandler(obj.transform.parent.gameObject);
        else return null;
    }
	...
	protected virtual void TryDamage()
	{
        if(m_Hit.collider.tag == "Zombie")
        {
            ZombieHandler zh = GetZombieHandler(m_Hit.transform.gameObject);
            zh.DoDamage((int)Damage);
        }
